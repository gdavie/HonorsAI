...
/**
 * This class is built to give an easy-to access communications bridge to libCPS
 * inputs required are aperature distance, frequencies,  and  the model
 * @author az
 *
 */
class CPSwrapper {
...
//	System.out.println("CPS time "+duration/ 1000000000.0);
  ...
  //  System.out.println("Print contents of array");
    ...
    public void callCPS(int aperature, double[] frequencies, double[] model){
    	CPSwrapper c = new CPSwrapper();
    	 double theory;    
         double r = aperature;
         int halfspace = model.length;
         //Add explicit halfspace
         model[halfspace++]=0.0000;
         model[halfspace++]=4.8600;
         model[halfspace++]=-9999; //terminaltion value
         
         c.runCPS(frequencies, model);
