...
c       20/01/2013
c
c       This contains some of the functionality of sdisp96.f
...
c-----
  ...
  600   continue
     ...
     1          verby,nfval,fval,ccmin,ccmax, numlayers)
     
c	 ilvry is hard-codes to 2 (Rayleigh)
c-----
...
c-----
        ...
        common /azphas/azcp(NPERIOD),azt0(NPERIOD)     
        ...
        !common/stor/ nlost,index,nroot1
        common/vels/ mvts(2),vts
...
c   11 format(1x,' improper initial value  no zero found. cmin=',f6.2)
c   10 format(' ' ,5x,4f10.4)
c   20 format(' ' ,15x,3f10.4)
c   30 format(' ' ,16x,'FLAT CRUSTAL MODEL USED  ' )
c   40 format(' ' ,7x,'   THICK    P-VEL     S-VEL    DENSITY')
...
c-----
	!write(6,*)'CMAX sdisp', cmax
        call mdsrch(cmin,cmax)
        !write(6,*)'CMAX sdisp', cmax
        if(ccmin.gt.0.0)cmin = ccmin
        !===TODO === This line kills the algorithm, may be
        ! 	A follow-on from the hack/simplifications made.
...
c-----
  ...
  410   continue
        ...
        ifunc=ilvry ! ilvry is hard-coded to 2 (Rayleigh)
c---------------------------------------
...
c-----
  !          kmode=0
  	  !write(6,*)'+++++',dcc(1), ifunc,omega0,omega1,cmin,cmax
            call poles(ifunc,omega0,omega1,cmin,cmax)
          ! write(6,*)'+++++',dcc(1), ifunc,omega0,omega1,cmin,cmax
c 4443   format('  Frequency(',i5,')=',1pe11.4,' Modes=',i5)
c 4444   format('+ Frequency(',i5,')=',1pe11.4,' Modes=',i5)
            kmode=nroot1
c-----
c            if(k.eq.1.and.kmode.eq.0) then
c            	    write(6,*),'==================='
c                iret = 0
c            endif
c            if(k.eq.1.and.kmode.eq.0) go to 3000
c            do 2100 i=1,kmode
            	!write(6,*)dcc(1)
                cp(1)=omega1/dcc(1)
                azcp(k)=cp(1)
