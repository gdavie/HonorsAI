...
public class SRP_Modify_add extends SRP_Modify{
	...
	public void execute(SProgram p1, SProgram p2, SProgram res) {
		...
		if(parameterToMod>=1){	//VS		
			modVal =  0 + (addSubRangeVS) * r.nextDouble();
			...
			modVal =  0 + (addSubRangeH) * r.nextDouble();
		...
		if(DEBUGMODIFY)System.out.println("Modify ADD called "+layerToMod+" : "+param+" : "+modVal);
	}
