...
public abstract class ManyProgram<MPR extends ManyProgram<MPR,IPR>,IPR extends IProgram<IPR>> 
		extends APartsProgram<MPR,IPR>{

	public IPopulation<?,IPR>[] sub_pops;
	private int selectedFactor[];
	private int id;
	/**normal constructor*/
	public ManyProgram(int prog_size, IPopulation<?,IPR>[] sub_pops, int numPieces, int numFeatures, int numRegisters){
	...
	public <X extends ManyProgram<X,IPR>> ManyProgram(ManyProgram<X, IPR> cp){
		super(cp);

		sub_pops = cp.sub_pops; //TODO this could be problematic. shared population at the moment
	}
	
	public void reinitialize(MPR cp){
	...
	public void pmReinitialize(MPR cp){
		super.reinitialize(cp);
		sub_pops = cp.sub_pops; //TODO this could be problematic. shared population at the moment
	}
	...
	public int selectedFactor(int index) {
		return selectedFactor[index];
	}

	public void setSelectedFactor(int index, int factor, int b) {
		selectedFactor [index]= factor;
	}
