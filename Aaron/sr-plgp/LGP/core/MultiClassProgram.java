...
public final class MultiClassProgram extends InstructionProgram<MultiClassProgram>{
	...
	//CACHING
	protected ExecutionTrace[] cache;
	//END CACHING

	/**normal constructor*/
	public MultiClassProgram(int programSize){
		...
		//CACHING
		if(config.CACHE){
			cache = new ExecutionTrace[config.numTrain + config.numValid];
			for(int i = 0; i < cache.length; i++){
				cache[i] = new ExecutionTrace(config.numCache,config.numRegisters);
			}
			modPos = 0;
		}
		//END CACHING
	...
	/**copy constructor*/
	public MultiClassProgram(MultiClassProgram rhs){
		super(rhs);
	...
	public MultiClassProgram clone(){
		return new MultiClassProgram(this);
	}

	public void reinitialize(MultiClassProgram rhs){
		// The fitnessMeasure should be copied in the sub-class which uses IProgram; 
		super.reinitialize(rhs);

		//MUTATIONJ
		distribution = new WeightedCollection<Integer>(((MultiClassProgram)rhs).distribution);
		for(int i = 0; i < config.numClasses; i++){
			resps[i] = ((MultiClassProgram)rhs).resps[i];
		}
		//END MUTATIONJ

		//CACHING
		if(config.CACHE){
			ExecutionTrace[] rhs_cache = ((MultiClassProgram)rhs).getCache();

			for(int i = 0; i < cache.length; i++){
				cache[i].copy(rhs_cache[i]);
			}
			modPos = rhs.modPos;
		}
		//END CACHING
	 ...
	 * @return
	 */
	public double[] diff(MultiClassProgram prog){
	...
	public void execute(FitnessEnvironment<?> fe, RegisterCollection rc, int num){
		...
		if(config.CACHE && fe.type != FitnessEnvironment.TEST){
			cacheNum = cache.length/2*fe.type + num;
			cachePos = (int)(config.numCache*((double)(modPos==0?0:modPos-1)/(double)config.maxLength));//TODO this has problems i think...

			if(cachePos > 0){
				cache[cacheNum].copyToRegisters(fe.getRegisters(), cachePos-1);
				pos = (int)(cachePos*(double)config.maxLength/(double)config.numCache) + 1;
			}
			else{
				pos = 0;
			}
		...
		for(int i = cachePos, var = pos; i < config.numCache+1; i++){//looping through all the instructions between cache points
			...
			// cache the register values every cacheInterval generations
			if(config.CACHE){
				if(fe.type != FitnessEnvironment.TEST && i < config.numCache){	
					cache[cacheNum].copyFromRegisters(fe.getRegisters(), i);
				}
			}
		}
	...
	public ExecutionTrace[] getCache(){
		return cache;
	}
