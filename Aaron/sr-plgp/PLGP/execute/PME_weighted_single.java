...
/** this uses the same weighting scheme as PME_weighted, but we do each program individually.
 * In other words we are only required to cache one program at once, find the weights, then 
 * discard and start on the next program.
 * @author Carlton
 *
 * @param <E>
 */
public class PME_weighted_single<E extends IProgram<E>> extends PME<E>{
	...
	public PME_weighted_single( int numRegisters, int numParts){
	...
	public <F extends IFitnessCase, APP extends APartsProgram> void updateFitness(FitnessEnvironment<F> tfe, FitnessEnvironment<F> vfe, APP ipp){
		...
		//update fitness a second time using weights
