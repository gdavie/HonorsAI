...
#include <time.h>
#include <stdio.h>
#include "libCPS_CPSwrapper.h"  // Required header for JNI
...
JNIEXPORT void JNICALL Java_libCPS_CPSwrapper_runCPS(JNIEnv *env,
...
//  shown here.
 ...
 //   printf("FREQ %d ", n);
    if(timers>0)printf("start JNI->c process\n");
    fflush(stdout);

    if(timers>0)printf("SPREP :: ");
    fflush(stdout);
    clock_t start = clock();
    sprep_(freqArray, mdlArray);
    clock_t end = clock();
    double div = CLOCKS_PER_SEC;
    if(timers>0)printf("%.4f seconds\n", (double)(end - start) / div);
    fflush(stdout);
...
**/
JNIEXPORT jdoubleArray JNICALL Java_libCPS_CPSwrapper_getFreq(JNIEnv *env,
                       jobject obj) {
	int i;
...
JNIEXPORT jdoubleArray JNICALL Java_libCPS_CPSwrapper_getC(JNIEnv *env,
                       jobject obj) {
	jdouble  c[NPERIOD];
	getc_(c);
	//Copy back to java
...
JNIEXPORT jdoubleArray JNICALL Java_libCPS_CPSwrapper_getEllip(JNIEnv *env,
                       jobject obj) {
